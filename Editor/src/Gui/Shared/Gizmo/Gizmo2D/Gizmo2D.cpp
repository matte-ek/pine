...
namespace
{
    ...
    void OnRender(Pine::RenderingContext *context, const Pine::RenderStage stage, float deltaTime)
    {
        ...
        if (stage == Pine::RenderStage::PostRender2D)
        {
            ...
            Pine::Renderer2D::SetCoordinateSystem(Pine::Rendering::CoordinateSystem::World);
    ...
    void RenderEntitySelectionBox(const Pine::Vector2f& viewportPositionOffset, const Pine::Vector2f& viewportSizeFactor)
    {
        ...
        for (auto& entity : selectedEntities)
        {
            if (auto spriteRenderer = entity->GetComponent<Pine::SpriteRenderer>())
            {
                ...
                const auto size = Pine::Vector2f(spriteRenderer->GetTexture()->GetWidth(), spriteRenderer->GetTexture()->GetHeight()) * Pine::Vector2f(entity->GetTransform()->GetScale()) / levelRenderingContext->Size;
...
void Gizmo::Gizmo2D::Render(const Pine::Vector2f& position, const Pine::Vector2f& size)
{
    ...
    const auto viewportScale =  Pine::Vector2f(size) / Pine::Vector2f(1920, 1080);
    ...
    if (gameRenderingContext->SceneCamera != nullptr)
    {
        ...
        const auto upperLeft = levelRenderingContext->SceneCamera->WorldToScreenPoint(gameCameraPosition + (Pine::Vector3f(-1.f, -1.f, 0.f) * gameRenderingContext->SceneCamera->GetOrthographicSize()));
        const auto bottomRight = levelRenderingContext->SceneCamera->WorldToScreenPoint(gameCameraPosition + (Pine::Vector3f(1.f, 1.f, 0.f) * gameRenderingContext->SceneCamera->GetOrthographicSize()));
