...
namespace
{
    ...
    {
        const auto embeddingModel = material->GetEmbeddingModel();

        if (embeddingModel != nullptr)
        {
            ImGui::TextDisabled("Embedded in the model '%s', and saved with it.", embeddingModel->GetPath().c_str());
            ImGui::TextDisabled("Re-importing the model resets its colors and textures from the file.");
            ImGui::Spacing();
        }
        ...
        if (assetModified)
        {
            // An embedded material has no file of its own: the model's '.passet' is what changed.
            if (embeddingModel != nullptr)
            {
                embeddingModel->MarkAsModified();
            }
            else
            {
    ...
    std::string FormatSampleDataSize(const std::size_t bytes)
    {
        ...
        constexpr auto megabyte = kilobyte * 1024.f;

        if (static_cast<float>(bytes) < megabyte)
        {
            return fmt::format("{:.1f} KB", static_cast<float>(bytes) / kilobyte);
        }
    ...
    void RenderAudioFile(Pine::AudioFile *audiofile)
    {
        ...
        {
            audiofile->ReImport();
            modifiedAudioFile = true;
        }

        if (modifiedAudioFile)
        {
            audiofile->MarkAsModified();
        }
    }
