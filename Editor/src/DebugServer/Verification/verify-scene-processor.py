"""Build a native probe of the scene processor's per-object caches with the Editor's boot sequence; requires a Ninja Editor build and Xvfb."""
...
root = Path(tempfile.mkdtemp(prefix='pine-scene-processor.'))
print('Native scene processor verification:', root, flush=True)
...
'''
body = Path(__file__).with_name('scene-processor-native.inc').read_text()
...
data = root / 'data'
(data / 'projects/scene/assets').mkdir(parents=True)
...
with log_path.open('w') as log:
    process = subprocess.Popen(headless_command(root / 'probe', 'scene'), cwd=data,
...
assert result == 0, 'Native scene processor verification failed; see ' + str(log_path)
