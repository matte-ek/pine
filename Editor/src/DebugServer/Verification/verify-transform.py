"""Build a native Transform hierarchy probe with the Editor's boot sequence; requires a Ninja Editor build and Xvfb."""
...
root = Path(tempfile.mkdtemp(prefix='pine-transform.'))
print('Native transform verification:', root, flush=True)
...
'''
body = Path(__file__).with_name('transform-native.inc').read_text()
...
data = root / 'data'
(data / 'projects/transform/assets').mkdir(parents=True)
...
with log_path.open('w') as log:
    process = subprocess.Popen(headless_command(root / 'probe', 'transform'), cwd=data,
...
assert result == 0, 'Native transform verification failed; see ' + str(log_path)
