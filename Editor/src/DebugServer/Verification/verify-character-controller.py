"""Build a native CharacterController probe with the Editor's boot sequence; requires a Ninja Editor build and Xvfb."""
...
root = Path(tempfile.mkdtemp(prefix='pine-character-controller.'))
print('Native character controller verification:', root, flush=True)
...
'''
body = Path(__file__).with_name('character-controller-native.inc').read_text()
...
data = root / 'data'
(data / 'projects/walker/assets').mkdir(parents=True)
...
with log_path.open('w') as log:
    process = subprocess.Popen(headless_command(root / 'probe', 'walker'), cwd=data,
...
assert result == 0, 'Native character controller verification failed; see ' + str(log_path)
