"""Build a native render-face probe and read a two-sided surface off the frame; requires a Ninja Editor build and Xvfb."""
...
root = Path(tempfile.mkdtemp(prefix='pine-render-face-native.'))
print('Native render face verification:', root, flush=True)
...
'''
body = Path(__file__).with_name('render-face-native.inc').read_text()
...
data = root / 'data'
(data / 'projects/renderface/assets').mkdir(parents=True)
...
with log_path.open('w') as log:
    process = subprocess.Popen(headless_command(root / 'probe', 'renderface'), cwd=data,
...
assert result == 0, 'Native render face verification failed; see ' + str(log_path)
