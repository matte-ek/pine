"""Build a native LOD probe and follow the level a model draws as it moves away from the camera; requires a Ninja Editor build and Xvfb."""
...
root = Path(tempfile.mkdtemp(prefix='pine-lod-native.'))
print('Native LOD verification:', root, flush=True)

# Use the same compiler flags and linked objects as Editor, replacing only its main-loop call with
# the probe: a scene /edit cannot build - nothing sets a model's LOD levels over HTTP - and then the
# same Run(), because what is being checked only exists once frames have been rendered.
...
'''
body = Path(__file__).with_name('lod-native.inc').read_text()
...
# it the Level viewport has no size and renders nothing to take statistics from.
data = root / 'data'
(data / 'projects/lod/assets').mkdir(parents=True)
...
assert result == 0, 'Native LOD verification failed; see ' + str(log_path)
