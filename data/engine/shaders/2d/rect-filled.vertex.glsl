...
void main()
{
    ...
    // Calculate rotation
    float rotY = sin(m_TextureIndexRadius.z);
    float rotX = cos(m_TextureIndexRadius.z);

    //vec2 position = vec2(m_Vertex.x * rotX - m_Vertex.y * rotY, 
    //                     m_Vertex.y * rotY - m_Vertex.x * rotX);
