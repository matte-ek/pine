...
- Each component type gets a **`ComponentDataBlock<T>`**: one contiguous array of instances plus a parallel occupation array, iterated by a custom iterator that skips empty/disabled slots. The array holds `m_MaxObjectCount` instances (default 32768) unless `Components::Setup()` passes a smaller count (`TerrainRendererComponent` and `Camera` get 32, `NativeScript` 1). New components take the first free slot, initialized by copying a default-constructed prototype. Overflow throws; code that must not throw, such as a C# binding, checks `Components::GetFreeSlotCount` / `Entities::GetFreeSlotCount` first.
- ⚠ **Occupy and free slots only through `ComponentDataBlock::MarkOccupied` / `MarkFree`** (and their entity equivalents in `Entities.cpp`). The block keeps its first free slot, its occupied count and the end of iteration up to date as slots change, rather than rescanning the whole pool. Writing to the occupation array directly would leave them stale.
