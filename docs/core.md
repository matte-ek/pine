...
## What's here
- **`Math/`** — vectors/matrices/quaternions (`Pine::Vector3f`, `Vector2i`, `Vector4d`, `Matrix4f`, `Quaternion`, etc.), built on GLM. `Math.hpp` is the umbrella include.
...
- **`Log/`** — logging. Use the macros `PInfo/PWarning/PError/PFatal(msg)`, not the functions directly. Each takes a single string, so format it yourself: `PInfo(fmt::format("Loaded {}", path))`. `PVerbose(msg)` is dropped unless verbose logging is on: set `PINE_VERBOSE` in the environment (the Editor and GameHost read it into `EngineConfiguration::m_VerboseLogging`) or call `Log::SetVerboseEnabled`. It is off by default because verbose messages - every asset load, every undo step - would push real warnings out of the 256-message history. To read the history back (the console panel, the debug server) call `Log::GetLogSnapshot()`: background threads log too, so it hands out a copy taken under the log's lock.
