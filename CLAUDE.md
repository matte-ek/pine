...
**Assets & shaders (`.passet` vs. source — read before touching either).** What the engine loads is the compiled **`.passet`**, built from raw source at import time — never hand-edit a `.passet`. A **shader**'s source is its `.glsl` files plus an **`.ih`** ("import hint") JSON sidecar next to the `.passet` (lists the `SourceFiles` and sampler bindings). Two distinct workflows, and mixing them up costs time:
- **Editing an existing** shader/asset: change the `.glsl`; **HotReload** (editor only) re-imports it on the fly when a tracked source's write-time changes. No manual step. (GameHost/production and fresh clones do **not** hot-reload — they need a build-time reimport.)
- **Adding a new** shader/asset: the engine only loads assets that already have a `.passet`; it will **not** auto-import unseen source. You must generate the `.passet` first with **`EngineCli --import <engine-path> <source files…>`** run from `data/` (it's headless — no GL context, no engine boot). Skipping this means `Assets::Get<T>()` returns null, and features that `assert` on their shader (e.g. `PostProcessing`, `AmbientOcclusion`, `Bloom`) will crash at boot — which also blocks the very import that would fix it. Commit the generated `.passet` + `.ih` alongside the source. Full details: [`docs/assets.md`](docs/assets.md).
